...
public function page(){
		...
		$User = M('category'); 
		// 实例化User对象
		$count      = $User->where('c_id=1')->join('inner join adminlog On category.addpeople_name=adminlog.admin_id')->order('ord desc')->count();
		// 查询满足要求的总记录数
		...
		$data['list'] = $User->where('c_id=1')->join('inner join adminlog On category.addpeople_name=adminlog.admin_id')->order('ord desc')->limit($Page->firstRow.','.$Page->listRows)->select();
